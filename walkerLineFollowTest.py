...
def calibrate():
    ...
    turningMotor.turn(-70, 60, brake = True, timeout=3, emulate=True)
    print("White = %d" % white)
    return (black,white)
...
def lineFollow():
    ...
    while (light.get_lightness() > 0):
        ...
        # ranges from -1 (at currLightness = black) to 1 (at currLightness = white)
        multiplier = 2.0 * (currLightness - threshold) / (white - black)
        print('%.4f' % multiplier)
        ...
        turningMotor.turn(d * turnPower, multiplier * maxTurn, brake = True)
