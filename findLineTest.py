...
def findLine(turnPower, threshold, black, white, delta):
    ...
    turningMotor.run(power = d * turnPower)
    while True:
        currLightness = light.get_lightness()
        #print(currLightness)
        if d == 1 and currLightness > threshold - delta:
            turningMotor.brake()
            return
        elif d == -1 and currLightness < threshold + delta:
            turningMotor.brake()
            return
...
def lineFollow():
    ...
    while True:
        findLine(turnPower, threshold, black, white, delta)
        sleep(1)
def calibrate():
